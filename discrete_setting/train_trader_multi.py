...
def train(total_eps: int, market_params: tuple, eval_freq: int, epsilon_start: float) -> DefaultDict:
    ...
    for episode in range(1, total_eps + 1):
        ...
        if episode % eval_freq == 0:
            ...
            for ttype in cumulative_stats:
                ...
                if ttype == 'RL':
                    RL_return.append(cumulative_stats[ttype]['avg_profit'])
                elif ttype == 'GVWY':
                    GVWY_return.append(cumulative_stats[ttype]['avg_profit'])
                elif ttype == 'ZIC':
                    ZIC_return.append(cumulative_stats[ttype]['avg_profit'])
                elif ttype == 'SNPR':
                    SNPR_return.append(cumulative_stats[ttype]['avg_profit'])
                elif ttype == 'ZIP':
                    ZIP_return.append(cumulative_stats[ttype]['avg_profit'])
                elif ttype == 'SHVR':
                    SHVR_return.append(cumulative_stats[ttype]['avg_profit'])
    ...
    return RL_return, GVWY_return, ZIC_return, SHVR_return, SNPR_return, ZIP_return

M = 3000 # number of episodes for monte carlo
number_of_policy_improvements = 20
CONFIG = {
...
buyers_spec = [('GVWY', 4), ('ZIC', 4), ('SHVR', 4), ('SNPR', 4), ('ZIP', 4)]
...
RL_return, GVWY_return, ZIC_return, SHVR_return, SNPR_return, ZIP_return = train(
...
# Plot the results
plt.plot(RL_return, label='RL', color='#085ea8')
plt.xlabel('Iterations')
plt.ylabel('Average Profit')
plt.savefig('discrete_RL.png')
plt.close()

plt.plot(GVWY_return, label='GVWY', color='#03045e')
plt.plot(RL_return, label='RL', color='#085ea8')
plt.plot(SHVR_return, label='SHVR', color='#7e95c5')
plt.plot(SNPR_return, label='SNPR', color='#eeadad')
plt.plot(ZIC_return, label='ZIC', color='#df676e')
plt.plot(ZIP_return, label='ZIP', color='#d43d51')
plt.xlabel('Iterations')
plt.ylabel('Average Profit')
plt.legend()
plt.savefig('discrete_GPI.png')
plt.show()
